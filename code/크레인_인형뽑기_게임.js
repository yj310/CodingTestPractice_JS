/*
    ...
    크레인을 모두 작동시킨 후 "터트려져 사라진 인형의 개수"를 return 하도록 solution 함수를 완성해주세요.
*/
    ...
    for(var i = 0; i < moves.length; i++) {
        ...
        for(var j = 0; j < board.length; j++){
            ...
            if(board[j][pos] != 0) {
                ...
                bucket.push(board[j][pos]);
                // console.log("add " + board[j][pos]);
                board[j][pos] = 0;

                if(bucket.length >= 2 && bucket[bucket.length-1] == bucket[bucket.length-2]) {
                    answer+=2;
                    // console.log(bucket);
                    // console.log(bucket[bucket.length-1]+""+ bucket[bucket.length-2]);
                    bucket.pop();
                    bucket.pop();
                    // console.log(bucket);
                    // console.log();
